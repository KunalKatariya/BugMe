...
class GoalsScreen extends ConsumerWidget {
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            ...
            toolbarHeight: 64,
            backgroundColor: bgColor,
            title: Text('Goals & Budget', style: tt.headlineMedium),
          ),
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  _SectionHeader(
                    ...
                    right: monthLabel,
                    tt: tt,
                    onAdd: () => _showAddBudget(context, ref, month),
                  ),
                  if (allocs.isEmpty)
                    Column(children: [
                      _EmptyCard(
                        emoji: '💰',
                        message: 'No budget set.\nTap below to add a category.',
                        cs: cs,
                        tt: tt,
                        isDark: isDark,
                      ),
                    ...
                    ...allocs.map((a) {
                      ...
                      final progress = budget > 0
                          ...
                          : 0.0;
                      final exceeded = spent > budget && budget > 0;
                      ...
                      return _BudgetCard(
                        ...
                        month: month,
                        onDelete: () =>
                            ref.read(databaseProvider).deleteAllocation(a.id),
                        onEdit: () =>
                            _showEditBudget(context, ref, a),
                      ).animate().fadeIn(duration: 300.ms);
                    }),
  ...
  Future<void> _showEditBudget(
      BuildContext context, WidgetRef ref, BudgetAllocation alloc) async {
    final ctrl   = TextEditingController(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Row(children: [
              Text(categoryEmoji(alloc.category),
                  style: const TextStyle(fontSize: 22)),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  await ref
                      .read(databaseProvider)
                      .updateAllocationAmount(alloc.id, amount);
                  if (ctx.mounted) Navigator.pop(ctx);
                },
  ...
  Future<void> _showAddBudget(BuildContext context, WidgetRef ref, String month) async {
    String? selectedCat;
    final ctrl   = TextEditingController();
    final cs     = Theme.of(context).colorScheme;
    final tt     = Theme.of(context).textTheme;
        ...
        builder: (ctx, setSt) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              Text('Add Budget Category', style: tt.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
              const SizedBox(height: 20),
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text(
                    '${formatAmount(spent, currency)} / ${formatAmount(alloc.allocatedAmount, currency)}',
                    style: tt.bodySmall
                        ?.copyWith(color: exceeded ? AppTheme.negative : null),
                  ),
                ],
              ),
              const SizedBox(height: 10),
              ...
              const SizedBox(height: 8),
              // ── On-track indicator + edit button row ────────────────
              Builder(builder: (context) {
                ...
                final expected = alloc.allocatedAmount > 0
                    ? alloc.allocatedAmount * now.day / daysInMonth
                    : 0.0;
                final String? trackLabel;
                final Color? trackColor;
                if (!isCurrentMonth || alloc.allocatedAmount == 0) {
                  trackLabel = null;
                  trackColor = null;
                } else if (exceeded) {
                  trackLabel = 'Over budget';
                  trackColor = AppTheme.negative;
                } else if (spent > expected) {
                  trackLabel = 'Over pace ⚠';
                  trackColor = Colors.orange;
                } else {
                  trackLabel = 'On track ✓';
                  trackColor = AppTheme.positive;
                }
                return Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
...
// ── Copy budget from last month button ────────────────────────────────────

class _CopyBudgetButton extends StatelessWidget {
  final String month;
  final ColorScheme cs;
  final TextTheme tt;
  final bool isDark;
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(14),
            border: Border.all(
                color: cs.onSurface.withAlpha(30), width: 1),
          ),
          child: Row(
            ...
            children: [
              Icon(Icons.copy_all_rounded,
                  size: 16, color: cs.onSurfaceVariant),
              const SizedBox(width: 8),
              Text('Copy budget from last month',
                  style: tt.bodySmall?.copyWith(
                      color: cs.onSurfaceVariant,
                      fontWeight: FontWeight.w600)),
            ],
